...
def rewrite_question(original_question):
    ...
    rewrites = {
        ...
        "预售订单延期发货：我的订单预计11月5日那周发货，但至今未收到发货通知，请问何时可以发货和收到货物？",

        # 3. 提前收货+延期原因
        "请问我能否提前收到我的电动自行车？我急需这辆自行车，而且11月底我要出门在外。非常感谢！另外，我还想问一下，为什么我下单后等了两个月才收到货？":
        "紧急发货咨询：我急需这辆电动自行车（11月底需要出行），能否提前发货？另外，为什么下单后等了两个月才收到货？",

        # 4. 电池更换
        "我们有一辆菲多D3 Pro，想知道有没有额外的电池，可以随时快速更换，开更远的距离。":
        "D3 Pro额外电池咨询：是否有额外电池可供购买，以便快速更换延长续航里程？",

        # 5. 儿童座椅兼容性
        "Thule Yepp Nexxt Maxi（后架安装版）。 请确认该座椅是否完全兼容我的Fiido自行车后货架？ 另外，若货架不适用，是否可以在车架上安装同款儿童座椅的车架固定版？":
        "儿童座椅兼容性：Thule Yepp Nexxt Maxi后架安装版是否兼容Fiido自行车后货架？如果不兼容，能否使用车架固定版安装？",

        # 6. 退货费用
        "退货政策页面上的说明不够清晰。如果自行车在购买后的30天内出现技术问题或质量问题，那么退货费用是否也需要由我承担呢？":
        "退货费用咨询：如果自行车在购买后30天内出现质量问题或技术问题，退货费用是否需要我承担？",

        # 7. T2产品现货
        "T2产品是否有现货？搭配脚踏板配件和儿童安全护栏的交货周期是多久？":
        "T2产品库存及配件：是否有现货？如果购买脚踏板配件和儿童安全护栏，交货周期是多久？",

        # 8. 儿童防雨配件
        "是否有配件能保护后排儿童免受雨水侵袭，比如大型硬顶遮阳篷？":
        "儿童防雨配件：是否有保护后排儿童免受雨淋的配件，比如大型硬顶遮阳篷？",
        ...
        "D3 Pro Mini配件咨询：Fiido自行车气筒座杆是否适用于这款车型？希望在圣诞节前收到车和配件。",
        ...
        "Nomads Touring标配：购买这款电动自行车是否包含脚撑、前后车灯、车铃和链罩？",

        # 15. T2升级
        "T2旧版本想升级为力矩版本可以吗":
        "T2升级咨询：旧版本能否升级为力矩版本？需要更换哪些部件？"
    }
    ...
    # 特殊处理：法国补贴申请问题（包含法律条文，优化但保留完整信息）
...
def process_and_rewrite(input_file, output_file):
    ...
    for idx, row in df.iterrows():
        ...
        if str(original) != str(rewritten):
            print(f"\n--- 问题 {idx + 1} 已改写 ---")
            print(f"原: {str(original)[:100]}{'...' if len(str(original)) > 100 else ''}")
            print(f"新: {str(rewritten)[:100]}{'...' if len(str(rewritten)) > 100 else ''}")
            rewritten_count += 1

        df.at[idx, '问题'] = rewritten
    ...
    print(f"改写完成！")
    print(f"总问题数: {len(df)}")
    print(f"改写问题数: {rewritten_count}")
    print(f"保持原样: {len(df) - rewritten_count}")
    print("=" * 80)
